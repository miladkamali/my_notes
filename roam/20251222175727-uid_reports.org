...
zcat http-service--31-12-2025-*|cut -d '|' -f5|grep "https://api.cloudcarbon.ir/carbon/v4.0/kyc_service/inquiry/national_id\""|jq 'select(.response.status == 200)|.response.body'|sed -e 's/\\"/"/g' -e 's/^"//' -e 's/"$//'|jq 'select(.result.gender=="female" and .result.alive=="false")|[.result.national_id,.result.birth_date,.tracking_number,.timestamp]| join(",")' >> dead_female_data.csv
zcat http-service--25-12-2025-*|cut -d '|' -f5|grep "https://api.cloudcarbon.ir/carbon/v4.0/kyc_service/inquiry/national_id\""|jq 'select(.response.status == 200)|.response.body'|sed -e 's/\\"/"/g' -e 's/^"//' -e 's/"$//'|jq 'select(.result.gender=="female" and .result.alive=="false")|.result.national_id'|wc -l
zcat http-service--25-12-2025-*|cut -d '|' -f5|grep "https://api.cloudcarbon.ir/carbon/v4.0/kyc_service/inquiry/national_id\""|jq 'select(.response.status == 200)|.response.body'|sed -e 's/\\"/"/g' -e 's/^"//' -e 's/"$//'|wc -l
...
#+begin_src gnuplot :var data=finotech-personal-info-daily-generator dead=dead-female-result :file finotech_daily_personal_info_stats.png
  reset
  set title "Daily Traffic: Success vs Failure"
  set terminal pngcairo size 1200,600 font "Arial,12"
  set style data histograms
  ...
  # Parse the Org-mode table data
  # 'data' variable automatically handles the array from the previous block
   plot dead using 2:xtic(1) title 'dead female' lc rgb "#cc0000", \
        '' using 3 title '200 response' lc rgb "#00cc00"#, dead  using 2:xtic(1) title 'dead-female' lc rgb "#110000"
  #plot dead using 2:xtic(1) title 'dead female' lc rgb "#cc0000", '' using 3 title '200' lc rgb "#00cc00"
#+end_src
