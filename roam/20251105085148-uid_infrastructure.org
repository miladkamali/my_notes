:PROPERTIES:
:ID:       7a0922df-453a-4788-bb9c-90d3ff52f163
:END:
#+title: UID infrastructure
#+STARTUP: overview
* taking back control of infrastructure                                :@uid:
** uid zytel ip
81.91.155.88
** email servers
iranserver.com
uid@iran.ir
Uid@1234
** uid shuttle ip
84.241.10.73
** create a dependency graph of our services

** infra git
https://gitlab2.uid.ir/services/docker-compose-file
https://gitlab2.uid.ir/infrastructure/platform-infra
ARGO_PASS|dUOTk0JWeWnTu
CI_DEPLOY_PASSWORD|RmU7YwopwCqpy4bohZMF
CI_REGISTRY|registry.uid.ir
MINIO_ACCESS_KEY|uid
MINIO_SECRET_KEY|3tVa3RqOcfRL
NPM_EMAIL|admin@example.org
NPM_PASSWORD|RW5SSVk0anQ=
NPM_USERNAME|admin
REGISTRY_PASS|DSsn8OmkaT9c

** get address of grafana
https://grafana.u-id.ir
admin
UPpXbm6oWN

http://46.245.89.2:3000/
admin
jXUdJR9cn9
*** DONE password of grafana
CLOSED: [2025-11-29 Sat 15:27]
:LOGBOOK:
- State "DONE"       from "DELEGATED"  [2025-11-29 Sat 15:27]
- State "DELEGATED"  from "TODO"       [2025-11-29 Sat 15:27]
- State "TODO"       from              [2025-11-24 Mon 20:58]
:END:
** elastic

elastic
KanUup6P9l
** get address of 
** argocd
https://argocd.u-id.ir/
admin
dUOTk0JWeWnTu
*** TODO how do i change argocd password
:LOGBOOK:
- State "TODO"       from              [2025-11-24 Mon 20:57]
:END:
** adminer
https://adminer.u-id.ir
** control of servers
*** get access to monitoring
**** elastic
http://46.245.89.2:5601/login?next=%2F
elastic
KanUup6P9l
** connecting to kubernates
*** dependencies
  - kubectl
  - k9s
*** credential
#+NAME: kube_certificate
#+BEGIN_SRC yaml
apiVersion: v1
clusters:
- cluster:
    certificate-authority-data: 
    server: https://k8s-lb.internal:6443
  name: kubernetes
contexts:
- context:
    cluster: kubernetes
    user: kubernetes-admin
  name: kubernetes-admin@kubernetes
current-context: kubernetes-admin@kubernetes
kind: Config
preferences: {}
users:
- name: kubernetes-admin
  user:
    client-certificate-data: 
    client-key-data: 

#+END_SRC


#+begin_src sh
sudo echo "46.245.89.9 k8s-lb.internal" >> /etc/hosts
#+end_src

#+RESULTS:
*** connection
k9s  --kubeconfig  ~/.kube/config

:namespace production
** DONE get access to vmwave  esx
CLOSED: [2025-11-26 Wed 13:05] SCHEDULED: <2025-11-07 Fri>
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-11-26 Wed 13:05]
- State "TODO"       from              [2025-11-07 Fri 10:38]
:END:

ssh -L 127.0.0.1:8444:192.168.100.4:443  milad@46.245.89.4 -p 3316 

https://127.0.0.1:8444
username:uid
password: Ec9yaqT9
password: xp#jMMwc8.'zJDp
** DONE monitoring status of servers
CLOSED: [2025-11-26 Wed 13:03] SCHEDULED: <2025-11-07 Fri>
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-11-26 Wed 13:03]
- State "TODO"       from              [2025-11-07 Fri 10:38]
:END:
** DONE uid vpn
CLOSED: [2025-11-18 Tue 11:29]
:LOGBOOK:
- State "DONE"       from "DOINGNOW"   [2025-11-18 Tue 11:29]
- State "DOINGNOW"   from "DELEGATED"  [2025-11-18 Tue 11:29]
- State "DELEGATED"  from "TODO"       [2025-11-07 Fri 11:23]
- State "TODO"       from              [2025-11-07 Fri 11:22]
:END:
*** DONE create local vpn for hasan
CLOSED: [2025-11-18 Tue 08:52] SCHEDULED: <2025-11-09 Sun>
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-11-18 Tue 08:52]
- State "TODO"       from              [2025-11-09 Sun 09:48]
:END:
*** DONE create local vpn for faraz
CLOSED: [2025-11-18 Tue 08:53] SCHEDULED: <2025-11-09 Sun>
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-11-18 Tue 08:53]
- State "TODO"       from              [2025-11-09 Sun 09:48]
:END:
*** DONE learn how can i create these vpns
CLOSED: [2025-11-18 Tue 09:52]
SCHEDULED: <2025-11-18 Tue>
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-11-18 Tue 09:52]
- Rescheduled from "[2025-11-09 Sun]" on [2025-11-18 Tue 08:53] \\
  just postponed to today
- State "TODO"       from              [2025-11-09 Sun 09:54]
:END:
ssh milad@10.4.4.41 -p 2222

/home/uid/openvpn-install.sh
cert password uid123
**** DONE figure out all the available users in the openvpn
CLOSED: [2025-11-19 Wed 08:34] SCHEDULED: <2025-11-18 Tue>
:LOGBOOK:
- State "DONE"       from "NEXT"       [2025-11-19 Wed 08:34]
- State "NEXT"       from              [2025-11-18 Tue 09:57]
:END:
     1) zara
     2) mzkamali
     3) milad-local
     4) soleymani
     5) hasan
     6) mk-local
     7) faraz-local
     8) hamed-local
**** DONE list all the users in 10.4.4.41 server
CLOSED: [2025-11-26 Wed 13:05] SCHEDULED: <2025-11-19 Wed>
:LOGBOOK:
- State "DONE"       from "NEXT"       [2025-11-26 Wed 13:05]
- State "NEXT"       from              [2025-11-18 Tue 10:09]
:END:
**** DONE figure out the config of microtik to expose this computer
CLOSED: [2025-11-26 Wed 13:05] SCHEDULED: <2025-11-19 Wed>
:LOGBOOK:
- State "DONE"       from "NEXT"       [2025-11-26 Wed 13:05]
- State "NEXT"       from              [2025-11-18 Tue 10:09]
:END:
** List of servers
https://conflu.uid.ir/pages/viewpage.action?spaceKey=DEV&title=asitech
*** 46.245.89.2 monitoring service
ssh milad@46.245.89.2 -p 3316

grafana
elkstack
monitoring
*** 46.245.89.4 ha proxy
/docker/ha/conf
*** 46.245.89.7 minio server
*** 46.245.89.9 gateway kubernates
*** DELEGATED 46.245.89.12 can only be accessed to from 46.245.89.9
:LOGBOOK:
- State "DELEGATED"  from              [2025-11-07 Fri 11:57]
:END:
*** DELEGATED 46.245.89.13
need to be set
:LOGBOOK:
- State "DELEGATED"  from              [2025-11-07 Fri 11:56]
:END:
*** 46.245.89.14
ssh milad@46.245.89.14 -p 3316
ca
sana
crypto
*** 46.245.89.15
authentication
simplified
bff
** TODO how do i reset passwords in ldap
SCHEDULED: <2025-11-18 Tue>
:LOGBOOK:
- State "TODO"       from              [2025-11-07 Fri 11:35]
:END:
** TODO learn about email server
:LOGBOOK:
- State "TODO"       from              [2025-11-09 Sun 10:31]
:END:
*** DONE fix email server
CLOSED: [2025-11-26 Wed 13:03] SCHEDULED: <2025-11-09 Sun>
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-11-26 Wed 13:03]
- State "TODO"       from              [2025-11-09 Sun 10:31]
:END:
** DONE get access to microtick
CLOSED: [2025-11-19 Wed 08:34] SCHEDULED: <2025-11-18 Tue>
:LOGBOOK:
- State "DONE"       from "NEXT"       [2025-11-19 Wed 08:34]
- State "NEXT"       from              [2025-11-18 Tue 08:54]
:END:
48:8F:5A:4D:40:1A
user milad
password: 58ed57e2-1d6d-461c-830d-135015689239
** DONE get access to hauwi router
CLOSED: [2025-11-26 Wed 13:05] SCHEDULED: <2025-11-18 Tue>
:LOGBOOK:
- State "DONE"       from "NEXT"       [2025-11-26 Wed 13:05]
- State "NEXT"       from              [2025-11-18 Tue 08:54]
:END:
** git and confluence
m.kamali
WY4d5E1EZFf2
** monitoring and alerting system
*** check about basic restart system
*** check about the alerts being changed
*** why i'm no longer receiving alerts
** asiatech panel
https://my.asiatech.ir/

14008858450
p~]4=w?p}ebB!+9u4P^3P~]Tn+f-R
** access to firewall
ssh kvm_user@178.22.120.146
pass: 13281328

connect serial

fortigate user admin
fortigate password 123
** access to minio
*** DONE get master password of minio
CLOSED: [2025-12-03 Wed 17:24]
ssh -L 127.0.0.1:9003:127.0.0.1:9003 milad@46.245.89.7 -p 3316
MINIO_ROOT_USER: c4wTRpjVbdeCL6yH
MINIO_ROOT_PASSWORD: "7wTKhrC]Q_Hbb3g6vnbC+guV3U._'z"

backup minio
ssh -N -L 9001:192.168.20.6:9001 -p 3316 milad@46.245.89.15
MINIO_ROOT_USER: uid
      MINIO_ROOT_PASSWORD: "3tVa3RqOcfRL"
SCHEDULED: <2025-11-29 Sat>
:LOGBOOK:
- State "DONE"       from "NEXT"       [2025-12-03 Wed 17:24]
- State "NEXT"       from              [2025-11-29 Sat 10:28]
:END:
** CI/CD

platform infra helm hcarts

issh milad@192.168.20.58 build service


all the docker compose are located at /docker/
** backup server
ssh milad@192.168.20.6 backup  server
it needs to be cleaned periodically 

minio
      MINIO_ROOT_USER: uid
      MINIO_ROOT_PASSWORD: "3tVa3RqOcfRL"
** HA
*** TODO figure out all the HA that actually do redirections
SCHEDULED: <2025-12-01 Mon>
:LOGBOOK:
- State "TODO"       from              [2025-12-01 Mon 06:41]
:END:
*** DELEGATED simplify HA of 4
SCHEDULED: <2025-12-01 Mon>
:LOGBOOK:
- State "DELEGATED"  from              [2025-12-01 Mon 06:43]
:END:
*** DELEGATED simplify HA of 14
SCHEDULED: <2025-12-01 Mon>
:LOGBOOK:
- State "DELEGATED"  from              [2025-12-01 Mon 06:43]
:END:
*** DELEGATED simplify HA of 15
SCHEDULED: <2025-12-01 Mon>
:LOGBOOK:
- State "DELEGATED"  from              [2025-12-01 Mon 06:43]
:END:
*** DELEGATED create a diagram of how things are being routed 
:LOGBOOK:
- State "DELEGATED"  from              [2025-12-01 Mon 06:44]
:END:
*** TODO List all the certificates and how we renew them
SCHEDULED: <2025-12-01 Mon>
:LOGBOOK:
- State "TODO"       from              [2025-12-01 Mon 06:44]
:END:
** kubernates
*** figure out how to add nodes
*** TODO figure out the storage structure
:LOGBOOK:
- State "TODO"       from              [2025-12-01 Mon 06:49]
:END:
*** TODO figure out backup and restore of database
:LOGBOOK:
- State "TODO"       from              [2025-12-01 Mon 06:50]
:END:
** DNS
*** TODO migrate all the dns to the email made for uid
DEADLINE: <2025-12-06 Sat>
:LOGBOOK:
- State "TODO"       from              [2025-12-03 Wed 13:07]
:END:
*** DONE get email user and password
CLOSED: [2025-12-14 Sun 18:25] DEADLINE: <2025-12-06 Sat>
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-12-14 Sun 18:25]
- State "TODO"       from              [2025-12-03 Wed 13:07]
:END:
** ILO

vpn info
L2tp preshared
address: idc-vpn.asiatech.ir
username: BINESH-HOOSHMAND
password: Binesh-Hooshmand@1544
password2 :Binesh-Hooshmand-Pishro@1544
Preshared Key : asiatech


| ILO ip            | server name |  server address | username      | password      | note                                    |
| * 172.17.174.238  | uid14       |                 | administrator | administrator |                                         |
| * 172.17.174.239  | ?           |                 |               |               | user and password are not administrator |
| *  172.17.174.231 | minio       |                 | administrator | administrator |                                         |
| * 172.17.174.226  | ?           |                 |               |               | cant connect seems it's not there       |
| 172.17.174.228    | esx         |   192.168.100.4 | administrator | administrator |                                         |
| * 172.17.174.233  | esx         | 192.168.100.209 | administrator | administrator |    binesh hooshmand 8                   |
| * 172.17.174.237  | esx         | 1921.168.100.13 | administrator | administrator |                                         |
|                   |             |                 |               |               |                                         |


binesh hooshmand-13     1000010278   LSR-BINESH-HOOSHMAND-PISHRO-SRV13-ILO          - 172.17.174.238
binesh hooshmand-14     1000010279   LSR-BINESH-HOOSHMAND-PISHRO-SRV14-ILO          - 172.17.174.239
binesh hooshmand-6      1000010283   LSR-BINESH-HOOSHMAND-PISHRO-SRV6-ILO           - 172.17.174.231
binesh hooshmand-7      1000010284
binesh hooshmand-9      1000010286
binesh hooshmand-4      1000010683
binesh hooshmand-5      1000010684



LSR-BINESH-HOOSHMAND-PISHRO-SRV3-P1            - 46.245.89.4

BINESH-HOOSHMAND-PISHRO-11    46.245.89.12
BINESH-HOOSHMAND-PISHRO-8    46.245.89.9    
BINESH-HOOSHMAND-PISHRO-12    46.245.89.13
BINESH-HOOSHMAND-PISHRO-14    46.245.89.15
BINESH-HOOSHMAND-PISHRO-1    46.245.89.2    
BINESH-HOOSHMAND-PISHRO-6    46.245.89.7    
BINESH-HOOSHMAND-PISHRO-3    46.245.89.4
لطفا IP ILOهای مشترک را اعلام نمایید . 
سپاس 
 

1405/04/30 17:23:40

با سلام
طبق اعلام واحد شبکه مرکز داده ( DCNM) :
Server 1 --- 172.17.174.226
Server 6 --- 172.17.174.231
Server 8 --- 172.17.174.233
Server 11 --- 172.17.174.236
Server 12 --- 172.17.174.237
Server 13 --- 172.17.174.238


1 89.2
srv03 pishro-3 46.245.89.4



pishro 6 89.7
pishro 8 89.9





46.245.89.12 --- 1000010278 --- binesh hooshmand-13
46.245.89.9 --- 1000010283 --- binesh hooshmand-6
46.245.89.13 --- 1000010284 --- binesh hooshmand-7
46.245.89.15 --- 1000010286 --- binesh hooshmand-9
46.245.89.2 --- 1000010279 --- binesh hooshmand-14
46.245.89.7 --- 1000010683 --- binesh hooshmand-4
46.245.89.4 --- 1000010684 --- binesh hooshmand-5
 

* key issues to be solved
** create a plan that we can detect issues with services in less than 5 minutes
** figure out how we can use N8N
** access to old logs
** access to old backups
** add users meta data to logs for easier tracking
** find a way to alert our customers for service outage
** integrating firewall with our infrastructure
** automate certificate renewal or use cloudflare 15 years certificate
* raw input

|    |          server | services                                                                                 | comment                                                  | ilo                                                                                                                                   |                            |                         |                      |                |
|  1 |     46.245.89.2 | log -monitoring                                                                          |                                                          | ?                                                                                                                                     |                            |                         |                      |                |
|  2 |     46.245.89.4 | internal services -vm                                                                    | vmware 192.168.100.4:443 username:uid password: Ec9yaqT9 |                                                                                                                                       |                            |                         |                      |                |
|  3 |     46.245.89.7 | minio                                                                                    |                                                          |                                                                                                                                       |                            |                         |                      |                |
|  4 |     46.245.89.9 | kuber                                                                                    |                                                          |                                                                                                                                       |                            |                         |                      |                |
|  5 |    46.245.89.12 | kuber                                                                                    |                                                          |                                                                                                                                       |                            |                         |                      |                |
|  6 |    46.245.89.13 | kuber                                                                                    |                                                          |                                                                                                                                       |                            |                         |                      |                |
|  7 |    46.245.89.14 | auth-simplified-bff                                                                      |                                                          |                                                                                                                                       |                            |                         |                      |                |
|  8 |    46.245.89.15 | sana-ca-bff-crypto                                                                       |                                                          |                                                                                                                                       |                            |                         |                      |                |
|  9 |   192.168.20.10 | vpn asiatec                                                                              | milad : 123                                              |                                                                                                                                       |                            |                         |                      |                |
| 10 |       10.4.4.41 | vpn-local-mail sertve  ssh port 2222  password openvpn uid123                            | /home/uid/openvpn-install.sh , mail is dokcer            | What is the mail package?                                                                                                             |                            |                         |                      |                |
| 11 |        10.4.4.1 | mikrotik                                                                                 | milad:764a6e5abfc99ccc                                   |                                                                                                                                       |                            |                         |                      |                |
| 12 |                 | ldap password reset                                                                      | http://192.168.20.60/phpldapadmin/                       | cn=admin,dc=uid,dc=ir , password:mRifIvnE                                                                                             |                            |                         |                      |                |
| 13 |                 | how are we managing ssl for u-id.net                                                     | https://spaceiran.com/ - cpanel generated                | user ?                                                                                                                                |                            |                         |                      |                |
| 14 |                 | how are we managing ssl for uid.ir                                                       | certbot with dns challeng                                | certbot   -d *.uid.ir -d uid.ir  --manual --preferred-challenges dns certonly --server https://acme-v02.api.letsencrypt.org/directory | server 4 or 2              |                         |                      |                |
| 15 |                 | ILO connection                                                                           |                                                          |                                                                                                                                       |                            |                         |                      |                |
| 16 |                 | Log pipelin                                                                              | 46.245.89.2                                              | /docker/docker-elk/logstash/pipeline/pipeline.d/                                                                                      |                            |                         |                      |                |
| 17 |    192.168.20.6 | Back up pipe line                                                                        |                                                          |                                                                                                                                       |                            |                         |                      |                |
| 18 |                 | registry                                                                                 | 192.168.20.5                                             |                                                                                                                                       |                            |                         |                      |                |
| 19 |                 | nexus and related vpn                                                                    | 192.168.20.5                                             | http proxy need  that get package                                                                                                     |                            |                         |                      |                |
| 20 |                 | the old backup of authentication (before removing logs)                                  | /home/hosein.11-01                                       |                                                                                                                                       |                            |                         |                      |                |
| 21 |                 | old logs and offline backups                                                             | eche service log is on own server                        | /home/pishro/log                                                                                                                      |                            |                         |                      |                |
| 22 |                 | elastic                                                                                  | http://46.245.89.2:5601                                  | elastic:KanUup6P9l                                                                                                                    |                            |                         |                      |                |
| 23 |                 | import logs from files to elastic                                                        | /docker/docker-elk/logstash/pipeline/pipeline.d/         |                                                                                                                                       |                            |                         |                      |                |
|    |                 | configuration network uid                                                                |                                                          |                                                                                                                                       |                            |                         |                      |                |
|    |                 | grafana user and password                                                                | http://46.245.89.2:3000                                  | admin:jXUdJR9cn9                                                                                                                      |                            |                         |                      |                |
|    |                 | migrate all the dns to uid specific email                                                | done                                                     |                                                                                                                                       |                            |                         |                      |                |
|    |                 | user and password of the email associated with uid DNS                                   | tecuid4@gmail.com,7ADarkE7m8@                            |                                                                                                                                       |                            |                         |                      |                |
|    |                 | Cleanup HA config of server 15 and 4 and 14                                              |                                                          |                                                                                                                                       |                            |                         |                      |                |
|    |                 | create a list of all the certificates that we use and their expiry time                  | 14-15-4-7-9-2                                            | 2months is expire time                                                                                                                |                            |                         |                      |                |
|    |                 | How i can access filesystem of kubernates( the one based on ceph)                        | https://ceph.u-id.ir/                                    | admin:uT28W"hQKbw#/-5-N]1S                                                                                                            |                            |                         |                      |                |
|    |                 | explain what each machine is about in VMWare( explain how you assign public IPs to them) |                                                          | ip addres is 4 and mange every thing in hapeoxy4 if need  expose service you sould add reverse proxy in haproxy 4                     |                            |                         |                      |                |
|    |                 | Create a diagram of what we have on each server(services )                               |                                                          | 15: sana-ca-bff-crypto                                                                                                                | 14: auth-simpillifiled-bff | 7: minio , 2:monitoring | 4 : internal service | 9, 12,13:kiber |
|    |                 | kubernates web dashboard bearer token                                                    |                                                          |                                                                                                                                       |                            |                         |                      |                |
|    |                 | how to update or generate new kube config                                                |                                                          |                                                                                                                                       |                            |                         |                      |                |
|    |                 |                                                                                          |                                                          |                                                                                                                                       |                            |                         |                      |                |
| 24 | user new server | milad : 123                                                                              |                                                          |                                                                                                                                       |                            |                         |                      |                |
| 25 |                 |                                                                                          |                                                          |                                                                                                                                       |                            |                         |                      |                |
* commands
docker exec -t psql-uid-crypto   pg_dump -U uid   -d uid_crypto_db > /home/pishro/backup/crypto/uid-crypto/db/test.sql






ip route add 192.168.20.0/24 via 192.168.100.20 dev eno2



sudo proxychains certbot certonly --dns-cloudflare -d "u-id.net,*.u-id.net"


supervisorctl  reread  &&  supervisorctl  update
vim /etc/supervisor/conf.d/ping-gost-1.conf
docker logs -f -n100 gost-ssh-tunnel
 cd /docker/d/
 vim docker-compose.yml


